 ...
 * Daniel Ye   2021/1/12  0.1.1      Support TrustKernel TEOS
 * Ashely Li   2021/1/15  0.2.0      Update copyright notice
 * David Tao   2021/1/18  0.2.1      Support TEEGris TEOS
 *
 *****************************************************************************/
...
#endif
#ifdef SECURITY_TYPE_TK
#include "tk/silead_tk_impl.h"
#endif
#ifdef SECURITY_TYPE_TEEGRIS
#include "teegris/silead_teegris_impl.h"
#endif
...
static pthread_mutex_t m_tz_lock = PTHREAD_MUTEX_INITIALIZER;
...
#ifdef SECURITY_TYPE_TK
    if(ret < 0) {
        ret = silfp_ca_tk_register(&m_ca_handler, ta_name);
    }
#endif
#ifdef SECURITY_TYPE_TEEGRIS
    if(ret < 0) {
        ret = silfp_ca_tg_register(&m_ca_handler, ta_name);
    }
#endif
#ifdef SECURITY_TYPE_NOSEC
    if (ret < 0) {
