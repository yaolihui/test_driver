...
int32_t silfp_cmd_auth_step(uint64_t op_id, uint32_t step, uint32_t is_pay, uint32_t *fid, uint32_t *flag)
{
    int32_t ret = -SL_ERROR_TA_OPEN_FAILED;
    if (m_fp_impl_handler != NULL) {
        if (m_fp_impl_handler->fp_auth_step != NULL) {
            ret = m_fp_impl_handler->fp_auth_step(op_id, step, is_pay, fid, flag);
        } else {
...
int32_t silfp_cmd_check_broken(worker_state_t state)
{
    ...
    if (!optic && (state == STATE_ESD_CHK || state == STATE_NAV)) {
        ...
        do {
            ...
            if (m_fp_impl_handler != NULL) {
                if (m_fp_impl_handler->fp_check_broken != NULL) {
                    silfp_dev_enable();
                    ret = m_fp_impl_handler->fp_check_broken(CHECK_BROKEN_GREY);
                    silfp_dev_disable();
                    if (ret != -SL_ERROR_TEST_TEMPER_RISE_FAILED) {
                        return SL_SUCCESS;
                    }
                } else {
            ...
            usleep(1000*300);
            LOG_MSG_ERROR("check broken ......retry times:%d",retry);
        } while (retry > 0);
    }
    return ret;
}
