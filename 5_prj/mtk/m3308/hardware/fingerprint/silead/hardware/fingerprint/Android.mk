...
#------------------------------------------------
# NOSEC(non security) TEE(trustonic) QSEE(qsee) GP(global platform) TRUSTY(trusty) OPTEE(op-tee) BEANPOD(beanpod) WATCH(watch) TK(tk) TEEGRIS(teegris) ALL(all)
ifeq ($(strip $(SEC_TYPE)),)
    SEC_TYPE = ALL
endif
...
ifeq ($(strip $(SEC_TYPE)),ALL)
    SEC_TYPE = QSEE TEE NOSEC GP TRUSTY OPTEE BEANPOD WATCH TK TEEGRIS
endif
...
ifeq ($(strip $(SIL_DUMP_TPL)),yes)
    LOCAL_CFLAGS_BASE += -DSIL_DUMP_TPL
endif
...
BASE_FILES_PATH := $(LOCAL_PATH) $(LOCAL_PATH)/log $(LOCAL_PATH)/silead_*
...
LOCAL_C_INCLUDES += $(LOCAL_PATH) \
                    $(LOCAL_PATH)/include \
                    $(LOCAL_PATH)/silead_misc/include \
