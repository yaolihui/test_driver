...
int32_t silfp_dev_wait_finger_status(int32_t irq, int32_t down, int32_t up, int32_t cancel, int32_t timeout)
{
    ...
    if (m_irq_op_mode == SIFP_FINGER_STATUS_ANDROID || (m_irq_op_mode == SIFP_FINGER_STATUS_TP)) {
    ...
    type = silfp_msg_wait_finger_status(irq, down, up, cancel, timeout);
    ...
    } else if (type == SIFP_MSG_TIMEOUT) {
        return -SL_ERROR_FINGER_STATUS_TIMEOUT;
    }
...
void silfp_dev_set_otp(uint32_t *otp)
{
    ...
    sprintf(buf, "0x%08X%08X%08X", otp[0], otp[1], otp[2]);
    LOG_MSG_ERROR("OTP_BUF:  %s\n", buf);
    ret = ioctl(m_dev_fd, SIFP_IOC_OTP_NODE, buf);
    if (ret < 0) {
        ret = -SL_ERROR_DEV_IOCTL_FAILED;
    }
}
