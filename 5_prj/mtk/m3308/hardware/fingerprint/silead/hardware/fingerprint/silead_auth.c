...
int32_t silfp_auth_command(void)
{
    ...
    if (ret >= 0) {
        do {
            ...
            if (silfp_impl_is_wait_finger_up_need() || silfp_impl_is_narrow_mode() || silfp_cust_need_wait_finger_up()) {
                ret = silfp_impl_wait_finger_up_with_cancel_impliment();
            }
            ...
            if (ret >= 0) {
                ...
                for (i = 0; i < retry_max; i++) {
                    ...
                    if (ret >= 0) {
                        ...
                        ret = silfp_impl_auth_step(m_auth_op_id, auth_step, m_is_pay_mode, (uint32_t *)&fid, &flag);
                        auth_step++;
                    ...
                    if (ret >= 0) { // matched
                        ...
                        if (ret_bak == -SL_ERROR_AUTH_MISMATCH) {
                            ret = ret_bak;
                        }
                    ...
                    if (silfp_worker_is_canceled()) {
                        LOG_MSG_DEBUG("auth cancel, stop retry authenticate.");
                        break;
                    }
                }
            }
            ...
            if (ret == -SL_ERROR_AUTH_MISMATCH) { // mismatch
                ...
                if ( silfp_cust_touch_by_mistake != NULL ) {
                    silfp_cust_touch_by_mistake(0, m_gid, m_auth_op_id, m_auth_id);
                } else {
                ...
                silfp_impl_get_auth_tpl_upd();
            }
            silfp_dump_data(DUMP_IMG_TYPE_AUTH);
            silfp_dump_tidy_data(DUMP_IMG_TYPE_AUTH, fid);
            if (ret >= 0) {
                break;
            }
