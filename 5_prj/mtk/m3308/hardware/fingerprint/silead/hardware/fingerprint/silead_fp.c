...
#define _tac_send_modified_command_2d_2r_and_get(cmd, p, l, d1, d2, r1, r2)      silfp_ca_send_modified_command(cmd, p, l, 2, d1, d2, r1, r2)
...
static int32_t fp_tac_auth_step(uint64_t op_id, uint32_t step, uint32_t is_pay, uint32_t *fid, uint32_t *flag)
{
    return _tac_send_normal_command_4d_2r(TZ_FP_CMD_AUTH_STEP, (uint32_t)(op_id >> 32), (uint32_t)op_id, step, is_pay, fid, flag);
}
...
static int32_t fp_tac_test_dump_data(uint32_t type, uint32_t step, void *buffer, uint32_t len, uint32_t *remain, uint32_t *size, uint32_t *w, uint32_t *h, uint8_t *bitcount)
{
    ...
    ret = _tac_send_modified_command_2d_2r_and_get(TZ_FP_CMD_TEST_DUMP_DATA, buffer, len, type, step, &data, &data2);
    if (ret >= 0) {
        if (w != NULL) {
            *w = (data2 & 0x0000FFFF);
        }
...
static int32_t fp_tac_check_broken(uint32_t mode)
{
    LOG_MSG_DEBUG("enter fp_tac_check_broken");

    return _tac_send_normal_command_1d(TZ_FP_CMD_CHECK_BROKEN, mode);
}
