...
static uint8_t m_img_mraw_thumbnail_enable = 1; // save thumbnail image in bmp data
static uint8_t m_img_24bit_rgb_enable = 1; // save rgb color in bmp data
static uint8_t m_img_is_encrypt = 0;       // bmp data is encrypt
static uint8_t m_img_is_dump = 0;       // bmp data is dump
...
{
    m_img_mraw_thumbnail_enable = enable;
}
...
{
    LOG_MSG_DEBUG("encrypt img type %x", enable);
    m_img_is_encrypt = enable;
}
...
static int32_t _bmp_get_img_fill_bmp_header_data(void *dest, uint32_t size, uint32_t w, uint32_t h, uint8_t bytes_per_pixel, uint32_t more_exts_size, uint16_t reserved)
{
    ...
    if ((m_img_16bit_type != IMG_16BIT_DATA_NORMAL) && (bytes_per_pixel == BMP_DATA_BYTE_16BIT)) {
        bf.bf_reserved1 = (m_img_16bit_type & 0x00FF);
    }
    LOG_MSG_DEBUG("m_img_is_encrypt %d, m_img_is_dump %d", m_img_is_encrypt, m_img_is_dump);
    if (m_img_is_encrypt && m_img_is_dump) {
        bf.bf_reserved1 |= 0xEE00;
    }
...
static int32_t _bmp_get_img_fill_bmp_data(void *dest, uint32_t size, uint32_t offset, const void *buf, uint32_t len, uint32_t w, uint32_t h, uint8_t bytes_per_pixel)
{
    ...
    silead_bmp_add_watermark(dest, offset, fixed_w, h, 1, 1);
...
static int32_t _bmp_mraw_thumbnail_get_img(void *dest, uint32_t size, const void *buf, uint32_t len, uint32_t w, uint32_t h, uint8_t bitcount, const void *extbuf, uint32_t extsize)
{
    uint8_t *pdest = (uint8_t *)dest;
    int32_t offset = 0;
    ...
    LOG_MSG_VERBOSE("get mraw thumbnail img data size: %d", offset);
...
static int32_t _bmp_mraw_thumbnail_parse_ext_data(uint32_t w, uint32_t h, const void *buf, uint32_t len, uint8_t bitcount, uint32_t bf_offbits, uint32_t offset)
{
    const uint8_t *p = (const uint8_t *)buf;
    bitmap_info_ext_header_t *bie = (bitmap_info_ext_header_t *)(p + offset);
...
static int32_t _bmp_mraw_thumbnail_get_data(void *dest, uint32_t size, uint32_t w, uint32_t h, const void *buf, uint32_t len, uint8_t bitcount)
{
    int32_t ret = 0;
    uint32_t bf_offbits = 0;
    uint8_t bi_bitcount = 0;
    ...
    LOG_MSG_VERBOSE("mraw_thumbnail parse");
    ...
    ret = _bmp_mraw_thumbnail_parse_ext_data(w, h, buf, len, bitcount, bf_offbits, data_offset);
    if (ret < 0) { // ext data invalid
        return -SL_ERROR_BAD_PARAMS;
    }
    data_offset += ret;
...
int32_t silfp_bmp_get_data_ext(void *dest, uint32_t size, uint32_t w, uint32_t h, const void *buf, uint32_t len, uint8_t bitcount, uint32_t *img_count)
{
    ...
    if (ret <= 0) {
        ret = _bmp_mraw_thumbnail_get_data(dest, size, w, h, buf, len, bitcount);
        if (ret > 0) {
            LOG_MSG_ERROR("get mraw_thumbnail data: ");
        }
    }
...
static int32_t _bmp_mraw_thumbnail_get_ext2(void *dest, uint32_t size, uint32_t w, uint32_t h, const void *buf, uint32_t len, uint8_t bitcount)
{
    int32_t ret = 0;
    uint32_t bf_offbits = 0;
    uint8_t bi_bitcount = 0;
...
int32_t silfp_bmp_get_ext2(void *dest, uint32_t size, uint32_t w, uint32_t h, const void *buf, uint32_t len, uint8_t bitcount)
{
    ...
    if (ret < 0) {
        ret = _bmp_mraw_thumbnail_get_ext2(dest, size, w, h, buf, len, bitcount);
    }
    if (ret < 0) {
...
static int32_t _bmp_mraw_thumbnail_get_size(const void *buf, uint32_t len, uint32_t *w, uint32_t *h)
{
    int32_t ret = 0;
    const uint8_t *p = (const uint8_t *)buf;
...
int32_t silfp_img_get_size(const void *buf, uint32_t len, uint32_t *w, uint32_t *h)
{
    ...
    if (ret < 0) {
        ret = _bmp_mraw_thumbnail_get_size(buf, len, w, h);
    }
...
int32_t silfp_bmp_save(const char *path, const char *name, const void *buf, uint32_t size, uint32_t w, uint32_t h, uint8_t bitcount, const void *extbuf, uint32_t extsize, uint32_t normal)
{
    ...
    if (count > 1) {
        if (m_img_mraw_thumbnail_enable) {
            len = _bmp_mraw_thumbnail_get_img(pbuff, len, buf, size, w, h, bitcount, extbuf, extsize);
