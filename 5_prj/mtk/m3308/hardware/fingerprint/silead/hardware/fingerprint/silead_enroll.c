...
int32_t silfp_enroll_command(void)
{
    ...
    if (ret >= 0) {
        do {
            ...
            silfp_cust_enroll_timeout(60);
            silfp_cust_clear_ui_ready();
            if (silfp_impl_is_wait_finger_up_need() && silfp_cust_need_wait_finger_up()) {
            ...
            if (ret < 0) {
                ...
                if (ret == -SL_ERROR_CANCELED) { // canceled
                    ...
                    LOG_MSG_DEBUG("notice_code=%d", notice_code);
                    switch (ret) {
                    ...
                    case -SL_ERROR_SAME_AREA: {
                        silfp_notify_send_acquired_notice(FINGERPRINT_ACQUIRED_GOOD);
                        silfp_notify_send_acquired_notice(notice_code);
                        silfp_cust_report_remaining(m_gid, remaining);
                        break;
                    }
            ...
            silfp_dump_data(DUMP_IMG_TYPE_ENROLL);
    ...
    if (silfp_impl_enroll_end(&fid) >= 0) { // return < 0, if not finish enroll or save tpl failed
        ...
        if(silfp_worker_is_canceled()){
			LOG_MSG_ERROR("fingerprint cancel,remove template");
			silfp_common_remove(m_gid, fid);
		} else {
            silfp_notify_send_acquired_notice(FINGERPRINT_ACQUIRED_GOOD);
    ...
    silfp_dump_tidy_data(DUMP_IMG_TYPE_ENROLL, fid);
    silfp_cust_enroll_timeout(0);
