...
typedef enum _req_cmd_id {
    ...
    REQ_CMD_SUB_ID_JUDGE_UPD_TPL            = 0x1011,   // judge update template (send and get)
    REQ_CMD_SUB_ID_ENABLE_DUMP_IMG          = 0x1012,   // enable dump img (send and get)
    REQ_CMD_SUB_ID_GET_TA_FEATURE           = 0x1013,   // get ta feature (send and get)
    REQ_CMD_SUB_ID_VERSION_EXT              = 0x1014,   // extend version (send and get)
    REQ_CMD_SUB_ID_DBG_ALGO_CFG             = 0x8004,   // dump algo config (send and get)
} req_cmd_id_t;
...
typedef enum _ext_cmd_id {
    ...
    EXT_CMD_IMAGE_TEST                  = EXT_CMD_BASE + 20,
...
enum _e_check_broken_mode_ {
    CHECK_BROKEN_GREY = 0,
    CHECK_BROKEN_DEADPIX = 1,
    CHECK_BROKEN_MUTIL = 2,
    CHECK_BROKEN_REPEAT = 3,
    CHECK_BROKEN_MAX,
};

typedef enum {
    TEST_IMAGE_ID_IRQ = 0,
    TEST_IMAGE_ID_CAPTURE,
    TEST_IMAGE_ID_QUALITY,
} test_image_id_t;

#define SL_ERROR_FINGER_STATUS_TIMEOUT 0x0FFFFF01
