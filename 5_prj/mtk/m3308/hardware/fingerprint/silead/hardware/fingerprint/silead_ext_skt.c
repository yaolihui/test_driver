...
static pthread_mutex_t m_ext_skt_data_lock = PTHREAD_MUTEX_INITIALIZER;
...
static int32_t _ext_skt_image_test(uint32_t cmd_id);
static const ext_skt_cmd_info_t ext_skt_cmd[] = {
    ...
    {EXT_CMD_IMAGE_TEST,        _ext_skt_image_test},
};
...
static int32_t _ext_get_versions_inter(void *buffer, uint32_t size)
{
    ...
    do {
        ...
        //add algo & ta version
        ret = silfp_impl_get_version_ext((uint8_t *)ver_ext, sizeof(ver_ext), (uint8_t *)ver, sizeof(ver));
        ret2 = silfp_ext_test_get_ta_ver(&algoVer, &taVer);
        if ((ret < 0) && (ret2 < 0)) {
            break;
        }

        if (((ret < 0) || (ver_ext[0] == '\0')) && (ret2 >= 0)) {
            snprintf(ver_ext, sizeof(ver_ext), "v%d", algoVer);
        }

        if (((ret < 0) || (ver[0] == '\0')) && (ret2 >= 0)) {
            snprintf(ver, sizeof(ver), "v%d", taVer);
        }
        offset = _ext_set_versions(buffer, offset, size, ver_ext, strlen(ver_ext), 0);
        offset = _ext_set_versions(buffer, offset, size, ver, strlen(ver), 0);
    } while (0);
...
static int32_t _ext_skt_test_otp(uint32_t cmd_id)
{
    ...
    if (ret >= 0) {
        ...
        if (ret >= 0) {
            ...
            if (ret <= (int32_t)offset) { // cust parse is n/a
                ret = silfp_impl_otp_parse(buffer, sizeof(buffer), offset, otp, NUM_ELEMS(otp));
            }
            if (ret < 0) {
                ret = offset;
            }
...
{
    int32_t ret = 0;
    uint8_t rsp[3] = {0}; // 0:id, 1:result, 2:quality
    char buf[256] = {0};

    pthread_mutex_lock(&m_ext_skt_data_lock);
    if (m_ext_skt_cmd_data.req_len >= m_ext_skt_cmd_data.offset + 1) {
        rsp[0] = (m_ext_skt_cmd_data.req[m_ext_skt_cmd_data.offset] & 0xFF);  // id
    }
    pthread_mutex_unlock(&m_ext_skt_data_lock);

    LOG_MSG_DEBUG("rsp[0]=%d", rsp[0]);
    ret = silfp_ext_capture_image_test(rsp[0], &rsp[2], &rsp[1]);
    if (ret >= 0) {
        ret = _ext_set_data_result(buf, sizeof(buf), rsp, sizeof(rsp));
    }

    if (ret >= 0) {
        _ext_skt_send_response(cmd_id, (int8_t *)buf, ret);
    } else {
        _ext_skt_send_error_notice(cmd_id, ret);
    }
