...
int32_t silfp_util_msec_to_date(uint64_t msec, char *datastr, uint32_t len, uint32_t mode, uint32_t format)
{
    ...
    if (days < (SL_DAYS_JAN_FEB + leapyear)) {
        ...
        month = SL_START_MONTH_9 + (days - SL_DAYS_BEFORE_SEP - leapyear) / SL_DAYS_PER_2_MONTH * 2 +
                ((((days - SL_DAYS_BEFORE_SEP - leapyear) % SL_DAYS_PER_2_MONTH) > SL_DAYS_PER_MONTH_30 - 1) ? 1 : 0);
        days = ((days - SL_DAYS_BEFORE_SEP - leapyear) % SL_DAYS_PER_2_MONTH) + 1;
        date = ((days > SL_DAYS_PER_MONTH_30) ? days - SL_DAYS_PER_MONTH_30 : days);
    }
...
int32_t silfp_ree_util_path_cat(void *dst, uint32_t size, const void *dir, uint32_t len, const void *subdir, uint32_t sublen)
{
    int32_t count = 0;
    int32_t count2 = 0;
    ...
    if ((dir == NULL) || (len == 0)) {
        return silfp_util_path_copy(pdata, size, subdir, sublen);
    }

    count = silfp_util_path_copy(pdata, size, dir, len);
    if (count <= 0) {
        return count;
    }

    if (count < (int32_t)(size - 1)) {
        pdata[count] = '/';
        pdata[count + 1] = '\0';
        count++;
    }
    ...
    count2 = silfp_util_path_copy(pdata + count, size - count, subdir, sublen);
    if (count2 > 0) {
        count += count2;
    }
