...
int32_t silfp_msg_wait_finger_status(int32_t irq, int32_t down, int32_t up, int32_t cancel, int32_t timeout)
{
    ...
    for (;;) {
        LOG_MSG_VERBOSE("poll iduc(%d,%d,%d,%d) timeout(%d)", irq, down, up, cancel, timeout);

        n = poll(&mPollFds, 1, timeout);
        if (n < 0) {
            if (errno == EINTR) {
                continue;
            }
            ...
            LOG_MSG_ERROR("poll timeout %d", timeout);
            return SIFP_MSG_TIMEOUT;
        }
