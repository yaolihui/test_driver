...
int fpc_tee_check_enrol_timeout(fpc_tee_t *tee, uint32_t enrol_timeout, uint8_t is_set)
{
    LOGD("%s: enrol_timeout: %d, is_set: %d", __func__, enrol_timeout, is_set);
    fpc_ta_hw_auth_command_t *command =
        (fpc_ta_hw_auth_command_t *) tee->shared_buffer->addr;

    pthread_mutex_lock(&tee->shared_mem_lock);
    command->header.command = FPC_TA_HW_AUTH_CHECK_ENROL_TIMEOUT;
    command->header.target = TARGET_FPC_TA_HW_AUTH;
    command->enrol_timeout.timeout_sec = enrol_timeout;
    command->enrol_timeout.is_set = is_set;
    int status = fpc_tac_transfer(tee->tac, tee->shared_buffer);

    LOGD("%s: enrol_timeout: %d", __func__, status);
    pthread_mutex_unlock(&tee->shared_mem_lock);
    return status;
}
#endif
