...
fpc_tee_t* fpc_tee_init(void)
{
    ...
    pthread_mutex_init(&tee->shared_mem_lock, NULL);
...
void fpc_tee_release(fpc_tee_t* tee)
{
    ...
    pthread_mutex_destroy(&tee->shared_mem_lock);
...
{
    return ((tee->supported_features >> FPC_TA_COMMON_AUTHENTICATOR_ENABLED) & 1) == 1;
}

bool fpc_tee_authenticator_2_enabled(const fpc_tee_t *tee)
{
    return fpc_tee_engineering_enabled(tee);
}
