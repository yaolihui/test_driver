...
static int get_cpu_id(void)
{
    ...
    char cpu_id[MAX_BUF_SIZE], ccpuid;
    char *p = NULL, *q = NULL;
    int i = 0;
    ...
    if (!file) {
        LOGE("%s Could not open /proc/%d/stat.\n", __func__, getpid());
        return -1;
    }

    int read_size = fread(cpu_id, 1, MAX_BUF_SIZE - 1, file);
    if (read_size <= 0) {
        LOGE("Unable to read data from file");
        return -1;
    }
    cpu_id[read_size] = '\0';
    ...
    if (q != NULL) {
        ccpuid = *q;
        LOGD("Fingerpint thread cpu id :%c", *q);
    }
...
static int get_cpu_freq(int cpu_id)
{
    FILE *file;
    char filepath[MAX_BUF_SIZE];
    char cpu_freq[MAX_BUF_SIZE];

    sprintf(filepath, "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_cur_freq", cpu_id);
    LOGD("Fingerpint thread cpu path :%s", filepath);

    file = fopen(filepath, "r");
    if (!file) {
        LOGE("%s Could not open cpu freq\n", __func__);
        return -1;
    }

    int read_size = fread(cpu_freq, 1, MAX_BUF_SIZE, file);
    if (read_size <= 0)
        LOGE("Unable to read data from file");

    if (file)
        fclose(file);
    return atoi(cpu_freq);
}
...
static int kpi_print_results(kpi_results_t *results, FILE *csv, int seq_num)
{
    ...
    int32_t fsd_total = 0;
    int32_t cac_total = 0;
    int cpuid = -1, cpufreq = 0;

    print_csv_metadata(csv);
    cpuid = get_cpu_id();
    cpufreq = get_cpu_freq(cpuid);
...
static void fpc_tee_kpi_next_sequence(fpc_tee_t *tee)
{
    ...
    if (size > KPI_RESULTS_SIZE * sizeof(kpi_results_t)) {
        free(results);
        goto out;
    }
