...
public class FingerprintSenseTouch extends FingerprintExtensionBase {
    ...
    public FingerprintSenseTouch() throws RemoteException {
        ...
        mFingerprintSenseTouch = IFingerprintSenseTouch.Stub.asInterface(getFingerprintExtension(SENSE_TOUCH));
        if (mFingerprintSenseTouch == null) {
            throw new RemoteException("Could not get " + SENSE_TOUCH);
        }
     ...
     * main thread.
     *
     * @param time        Stable time needed to finish calibration
     * @param sensitivity If the value fluctuates above or below the given sensitivity,
    ...
    public boolean readConfig(SenseTouchConfig senseTouchConfig) {
        ...
        if (mFingerprintSenseTouch != null) {
            try {
                status = mFingerprintSenseTouch.readConfig(senseTouchConfig);
            } catch (RemoteException e) {
                mLogger.e("RemoteException: ", e);
            }
        } else {
