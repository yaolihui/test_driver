...
status_t BnFingerprintNavigation::onTransact(uint32_t code, const Parcel& data,
    ...
    switch (code) {
        ...
        case GET_NAVIGATION_CONFIG: {
            ...
            reply->writeInt32(config.threshold_tap_n_images_max);
            reply->writeInt32(config.threshold_hold_n_images_min);
            reply->writeInt32(config.double_click_time_interval);
            reply->writeInt32(config.threshold_tap_translation_max);
            reply->writeInt32(config.threshold_swipe_translation_min);
            reply->writeInt32(config.mode);
            return NO_ERROR;
        }
        case SET_NAVIGATION_CONFIG: {
            ...
            if (parcel_header == HAS_PARCEL_DATA) {
                config.threshold_tap_n_images_max = data.readInt32();
                config.threshold_hold_n_images_min = data.readInt32();
                config.double_click_time_interval = data.readInt32();
                config.threshold_tap_translation_max = data.readInt32();
                config.threshold_swipe_translation_min = data.readInt32();
                config.mode = data.readInt32();
