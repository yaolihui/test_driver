...
status_t BnFingerprintAuthenticator::onTransact(uint32_t code,
    ...
    switch (code) {
        case VERIFY_USER: {
            ...
            const ssize_t fido_auth_input_1_len = data.readInt32();
            const uint8_t* fido_auth_input_1 =
                    static_cast<const uint8_t *>(data.readInplace(fido_auth_input_1_len));
            const ssize_t fido_auth_input_2_len = data.readInt32();
            const uint8_t* fido_auth_input_2 =
                    static_cast<const uint8_t *>(data.readInplace(fido_auth_input_2_len));
            ...
            verifyUser(callback, fido_auth_input_1, fido_auth_input_1_len,
                       fido_auth_input_2, fido_auth_input_2_len,
                       dstAppName, dstAppNameLength);
