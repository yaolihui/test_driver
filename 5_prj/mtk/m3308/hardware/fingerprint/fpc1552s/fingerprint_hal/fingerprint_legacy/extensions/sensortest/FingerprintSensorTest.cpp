...
void FingerprintSensorTest::testOnResult(
    ...
    fpc_hal_ext_sensortest_test_result_t res;
    if (!result) {
        res.result_code = FPC_HAL_EXT_SENSORTEST_TEST_ERROR;
        strcpy(res.error_string, "FPC_HAL_EXT_SENSORTEST_TEST_ERROR");
        res.error_code = -1;
        strcpy(res.result_string, "FPC_HAL_EXT_SENSORTEST_TEST_ERROR");
        result = &res;
    }
