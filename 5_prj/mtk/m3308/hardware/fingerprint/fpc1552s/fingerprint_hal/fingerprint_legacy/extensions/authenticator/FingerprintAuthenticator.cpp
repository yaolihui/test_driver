...
void FingerprintAuthenticator::verifyUser(
        const sp<IVerifyUserCallback>& callback,
        const uint8_t* fido_auth_input_1, ssize_t fido_auth_input_1_len,
        const uint8_t* fido_auth_input_2, ssize_t fido_auth_input_2_len,
        const char* dstAppName, ssize_t dstAppNameLength) {
    ...
    if (mDevice) {
        uint32_t status = mDevice->verify_user(
                mDevice, fido_auth_input_1, fido_auth_input_1_len,
                fido_auth_input_2, fido_auth_input_2_len,
                dstAppName, dstAppNameLength, this,
