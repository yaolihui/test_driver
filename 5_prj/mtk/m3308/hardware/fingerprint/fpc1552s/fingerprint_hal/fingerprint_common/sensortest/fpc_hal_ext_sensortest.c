...
#define ESD_CHECK_PRE_TEST_NAME "ESD Check Pre Test"
#define ESD_CHECK_TEST_NAME "ESD Check Test"
#define ESD_CHECK_POST_TEST_NAME "ESD Check Post Test"
...
#define ESD_CHECK_PRE_TEST_DES "Read all of the RW registers and store those"
#define ESD_CHECK_TEST_DES "Verifies latch up or bit-shift of sensor internal status and must run ESD Check Pre first"
#define ESD_CHECK_POST_TEST_DES "If ESD test fail and resume by reset the sensor and download the registers dumped before"
...
typedef enum {
    FPC_SENSORTEST_DOMAIN_CAPACITIVE,
    FPC_SENSORTEST_INVALID_DOMAIN,
} sensortest_domain_t;
...
#define TEST_ENTRY(domain, test) \
{ \
    FPC_SENSORTEST_DOMAIN_ ## domain, \
...
#define TEST_ENTRY_WITH_CONFIG(domain, test, get_config) \
{ \
    FPC_SENSORTEST_DOMAIN_ ## domain, \
    FPC_SENSORTEST_ ## test, \
    test ## _NAME, \
...
#define TEST_ENTRY_STAMP(domain, test, get_config) \
{ \
    FPC_SENSORTEST_DOMAIN_ ## domain, \
    FPC_SENSORTEST_ ## test, \
    test ## _NAME, \
...
const struct {
    ...
    TEST_ENTRY(CAPACITIVE, SELF_TEST),
    TEST_ENTRY(CAPACITIVE, CHECKERBOARD_TEST),
    TEST_ENTRY_STAMP(CAPACITIVE, IMAGE_QUALITY_TEST, NULL),
    TEST_ENTRY(CAPACITIVE, RESET_PIXEL_TEST),
    TEST_ENTRY(CAPACITIVE, AFD_CALIBRATION_TEST),
    TEST_ENTRY(CAPACITIVE, AFD_CAL_TEST),
    TEST_ENTRY_STAMP(CAPACITIVE, MODULE_QUALITY_TEST, get_mqt_config),
    TEST_ENTRY(CAPACITIVE, DEFECTIVE_PIXELS_TEST),
    TEST_ENTRY(CAPACITIVE, OTP_VALIDATION_TEST),
    TEST_ENTRY(CAPACITIVE, TXPULSE_CHECKERBOARD_TEST),
    TEST_ENTRY(CAPACITIVE, ESD_CHECK_PRE_TEST),
    TEST_ENTRY(CAPACITIVE, ESD_CHECK_TEST),
    TEST_ENTRY(CAPACITIVE, ESD_CHECK_POST_TEST),
    {FPC_SENSORTEST_INVALID_DOMAIN, FPC_SENSORTEST_INVALID_TEST, NULL, NULL, NULL, false, NULL }
};
...
static void do_capture(void *self)
{
    ...
    if (module->uncalibrated) {
        ...
        for (;;) {
            status = fpc_tee_capture_image(module->hal->sensor, 1);
        ...
        status = fpc_tee_capture_image(module->hal->sensor, 0);
