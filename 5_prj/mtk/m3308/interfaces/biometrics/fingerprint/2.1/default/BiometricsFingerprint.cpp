...
#define FINGERPRINT_SILEAD_HARDWARE_MODULE_ID       "fingerprint.silead"
...
static const char *variant_keys[] = {
	...
	FINGERPRINT_SILEAD_HARDWARE_MODULE_ID,
#else
	FINGERPRINT_FPC_HARDWARE_MODULE_ID,
#endif
};

static const int FINGER_NUM = (sizeof(variant_keys)/sizeof(variant_keys[0]));
// WIKO END
using RequestStatus =
...
// xushengjie, DATE20220322, V820, add for fingerprint bringup
    ...
    for(i = 0; i < FINGER_NUM; i++) {
        ALOGE("fingerprint open hal i = %d", i);
        ALOGE("Module ID = %s", variant_keys[i]);
...
void BiometricsFingerprint::notify(const fingerprint_msg_t *msg) {
	...
	ALOGE("%s, YLH: msg->type=%d", __func__, msg->type);
    switch (msg->type) {
        case FINGERPRINT_ERROR: {
                int32_t vendorCode = 0;
                FingerprintError result = VendorErrorFilter(msg->data.error, &vendorCode);
                ALOGE("YLH:onError(%d), in(%d), vendorCode(%d)", result, msg->data.error, vendorCode);
        ...
        case FINGERPRINT_ACQUIRED: {
                ...
                ALOGE("YLH:onAcquired(%d), in(%d), vendorCode(%d)", result, msg->data.acquired.acquired_info, vendorCode);
