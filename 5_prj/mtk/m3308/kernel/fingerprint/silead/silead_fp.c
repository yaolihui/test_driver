...
#ifdef ROMANEE_FP
//shangfei add fp_version for silead 2022/6/15 begin
extern char fp_version[32];
//shangfei add fp_version for silead 2022/6/15 end
#endif

//yaolihui add fp_id for silead 2022/7/1 begin
#ifdef CONFIG_OTP_SUPPORT
extern char fp_id[64];
static char otp_data[OTP_DATA_LEN];
#endif
//yaolihui add fp_id for silead 2022/7/1 end
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case SIFP_IOC_DEINIT:
        ...
        #ifdef ROMANEE_FP
        //shangfei add fp_version for silead  2022/6/15 begin
        LOG_MSG_DEBUG(ERR_LOG, "write 'fp not found' to fp_version \n");
	    memset(fp_version,0,sizeof(fp_version));
        snprintf(fp_version, sizeof(fp_version),"%s","fp not found");
        //shangfei add fp_version for fpsensor 2022/6/15 end
        #endif
        break;
    ...
    case SIFP_IOC_RESET:
        ...
        break;
#ifdef CONFIG_OTP_SUPPORT
	case SIFP_IOC_OTP_NODE:
		if (arg) {
			if (copy_from_user(otp_data, (void __user *)arg, OTP_DATA_LEN)) {
				LOG_MSG_DEBUG(ERR_LOG, "[SIFP_IOC_OTP_NODE] copy_from fail\n");
				retval = -EFAULT;
				break;
			}
			LOG_MSG_DEBUG(ERR_LOG, "YLH:OTP_DATA:	%s\n", otp_data);
			memset(fp_id, 0, sizeof(fp_id));
			strcpy(fp_id, otp_data);
			LOG_MSG_DEBUG(INFO_LOG, "YLH:after  write module_id to fp_id:[%s]\n", fp_id);
		}
		break;
#endif
...
static int silfp_probe(struct spi_device *spi)
{
    ...
    #ifdef ROMANEE_FP
    //shangfei add fp_version for silead  2022/6/15 begin
    LOG_MSG_DEBUG(ERR_LOG, "write chipinfo to fp_version \n");
	memset(fp_version,0,sizeof(fp_version));
    snprintf(fp_version, sizeof(fp_version),"%s","GSL6159");
    //shangfei add fp_version for fpsensor 2022/6/15 end
	return status;
    #endif
