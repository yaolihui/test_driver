...
#define FPSENSOR_TBASE_COMPATIBLE        1   // 0:off    1: on
...
#define FPSENSOR_IOC_INFO           			_IOWR(FPSENSOR_IOC_MAGIC,23,unsigned char[64])
#endif
///yaolihui add fp_id for fpsensor 2022/7/1 end


#define GET_CHIP_INFO  1
#ifdef	GET_CHIP_INFO
...
typedef struct {
    dev_t devno;
    struct class *class;
    struct device *dev;/*shangfei add for config regulator power supply for fpsensor 2022/5/28*/
    struct cdev cdev;
#if USE_SPI_BUS
    ...
    u16 spi_freq_khz;
    #endif
    struct regulator *vdd;/*shangfei add for config regulator power supply for fpsensor 2022/5/28*/
} fpsensor_data_t;
